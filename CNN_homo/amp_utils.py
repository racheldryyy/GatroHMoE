...
class AmpHandler:
    ...
    def __init__(self, enabled=True, init_scale=2**16, growth_factor=2.0, 
        ...
        print(f"[DEBUG] AMP初始化 - enabled={enabled}")
        print(f"[DEBUG] CUDA可用性: {torch.cuda.is_available()}")
        if torch.cuda.is_available():
            print(f"[DEBUG] GPU数量: {torch.cuda.device_count()}")
            print(f"[DEBUG] 当前GPU: {torch.cuda.current_device()}")
        ...
        if self.enabled and not torch.cuda.is_available():
            print("[WARNING] CUDA不可用，AMP将被禁用!")
            self.enabled = False
            self.scaler = GradScaler(enabled=False)
        else:
            print(f"[DEBUG] AMP启用状态: {self.enabled}")
