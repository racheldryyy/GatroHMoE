...
def evaluate_model(model, data_loaders, model_name):
    ...
    for task, task_data in data_loaders.items():
        ...
        with torch.no_grad():
            for inputs, labels in test_loader:
                ...
                # 正确处理DataParallel包装的模型
                if hasattr(model, 'module'):  # DataParallel wrapped model
                    if isinstance(model.module, (MixtureOfExperts, HeterogeneousMixtureOfExperts)):
                        outputs, routing_weights = model(inputs, task=task)
                    else:  # Student model or other models wrapped in DataParallel
                        outputs = model(inputs, task=task)
                elif isinstance(model, (MixtureOfExperts, HeterogeneousMixtureOfExperts)):
                    outputs, routing_weights = model(inputs, task=task)
    ...
    if isinstance(model, (MixtureOfExperts, HeterogeneousMixtureOfExperts)):
        ...
        if isinstance(model, HeterogeneousMixtureOfExperts) and hasattr(model, 'expert_names'):
...
def analyze_expert_usage(model, data_loaders, model_name):
    ...
    with torch.no_grad():
        for task, task_data in data_loaders.items():
            ...
            for inputs, _ in test_loader:
                ...
                # Get routing weights - handle DataParallel wrapped models
                if hasattr(model, 'module'):  # DataParallel wrapped model
    ...
    if isinstance(model, HeterogeneousMixtureOfExperts) and hasattr(model, 'expert_names'):
        expert_labels = [f"{i+1}: {name}" for i, name in enumerate(model.expert_names)]
    else:
...
def analyze_expert_performance(model, data_loaders, model_name):
    """分析每个专家的单独性能"""
    if not isinstance(model, HeterogeneousMixtureOfExperts) or not hasattr(model, 'expert_names'):
