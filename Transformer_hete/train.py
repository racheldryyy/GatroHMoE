...
matplotlib.use('Agg')  # 使用无界面后端，适合服务器环境
import pandas as pd
...
from config import (
    ...
    TASK_WEIGHT_STRATEGY, TASK_WEIGHT_ALPHA,BATCH_SIZE, INPUT_SIZE
)
from utils import (
...
def train_teacher_model(base_model_name='resnet50', resume=False, use_amp=USE_AMP):
    """训练教师模型 - 优化版本"""
    # 设置随机种子
    set_seed(SEED)
    
    # 创建结果目录
    log_dir = os.path.join(RESULT_PATH, "logs", f"teacher_{base_model_name}")
    ...
    for task in task_weights.keys():
        train_loader = data_loaders[task]['train_loader']
        # 获取样本数
    ...
    # 创建MoE模型
    print(f"创建MoE模型 (教师模型: {base_model_name})...")
    ...
    if torch.cuda.device_count() > 1:
        print(f"✅ 启用 {torch.cuda.device_count()} 块GPU进行并行计算")
        # 验证GPU设备ID有效性
